...
class create_ic():
	...
	def create_ic_regional_llc(self,variable,datasets_facets,fileout,timestep=None,nx=270,clim=False,\
		...
		self.nt, self.ny = self._compute_dimensions(datasets_facets)
	...
	def _create_output_array(self,clim=False):
		if clim:
			placeholder = np.empty((self.nt,self.ny,self.nx))
		else:
			placeholder = np.empty((1,self.ny,self.nx))
		return placeholder
	...
	def _reorder_array(self,datain):
		...
		if ndims == 3:
			...
			dataout = np.empty((nt,nx,ny))
			for kt in np.arange(nt):
				dataout[kt,:,:] = np.reshape(datain[kt,:,:],[nx,ny])
		if ndims == 2:
			ny,nx = datain.shape
			dataout = np.empty((nx,ny))
			dataout[:,:] = np.reshape(datain[:,:],[nx,ny])
